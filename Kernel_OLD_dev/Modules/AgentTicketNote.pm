...
sub _Mask {
    ...
    if ( $Self->{Config}->{Note} ) {
		...
		$Self->{LogObject}->Log( Priority => 'info', Message => "Called AgentTicketNote -> get ArticleTypeList" );
        # get possible notes
		...
		#for my $Key ( keys %CornerNoteTypes ) {
		#   if( $CornerNoteTypes{$Key} ){
		#		$Self->{LogObject}->Log( Priority => 'error', Message => "--> $Key $CornerNoteTypes{$Key}" );
		#   }
		#}
