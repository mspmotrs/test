...
sub Run {
    ...
    if ( !$Access ) {
				$Self->{TicketObject}->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Error updating priority: no $Self->{Config}->{Permission} permissions on TicketID $Self->{TicketID} for the UserID $Self->{UserID}.",
						UserID		=>	$Self->{UserID},
						TicketID	=>	$Self->{TicketID},
						Body			=>	'No',
						Result		=>	'KO',
    ...
    if ( $Self->{Config}->{RequiredLock} ) {
        if ( !$Self->{TicketObject}->LockIsTicketLocked( TicketID => $Self->{TicketID} ) ) {
            ...
            if ( !$AccessOk ) {
								$Self->{TicketObject}->AuditTicketAction (
										Action		=>	'UPDATE',
										Message		=>	"Priority action failed because userID $Self->{UserID} is not the owner of the ticket $Self->{TicketID}.",
										UserID		=>	$Self->{UserID},
										TicketID	=>	$Self->{TicketID},
										Body			=>	'No',
										Result		=>	'KO',
    ...
    if ( $Self->{Subaction} eq 'Store' ) {
        ...
        if ( $Self->{Config}->{State} && $GetParam{NewStateID} ) {
            ...
            if ( $StateData{TypeName} =~ /^close/i ) {
                $Self->{TicketObject}->AuditTicketAction (
										Action		=>	'UPDATE',
										Message		=>	"Priority notified for TicketID $Self->{TicketID} by UserID $Self->{UserID}.",
										UserID		=>	$Self->{UserID},
										TicketID	=>	$Self->{TicketID},
										Body			=>	'No',
										Result		=>	'OK',
