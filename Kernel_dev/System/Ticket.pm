...
sub CustomerMoveTicket {
    ...
    if( $notifyConf{$Queue} ){
		$Self->{LogObject}->Log( Priority => 'error', Message => "Called ----> ADAPTER_INTERFACE_2 <----" );
...
# $Self->{LogObject}->Log(
#		 Priority        => 'info',
#		 Message         => "PIPPOPIPPOPIPPOPIPPOPIPPOPIPPOPIPPOPIPPO:".$Queue.":NNNNNN:".$notifyConf{$Queue}.":TTTTT:".$Param{ParentUpd},
#	);

 
    if( $notifyConf{$Queue} ){
		$Self->{LogObject}->Log( Priority => 'error', Message => "Called ----> ADAPTER_INTERFACE_3 <----" );
...
sub TicketTypeList {
    ...
    if ($ACL) {
        #while ((my $k, my $v) = each(%Types)){
		#	 $Self->{LogObject}->Log ( Priority	=> 'error',  Message => "############## ".$k." -> ".$v);
		#}
		return $Self->TicketAclData();
    }
...
$Self->{LogObject}->Log( Priority => 'error', Message => "Before Planned Date ::: $Ticket{TicketNumber} " );
    ...
    if( $Param{Counter} eq 4 ){
      ...
      if ( !$Param{ParentUpd} || $Param{ParentUpd} ne '1' ) { 
		$Self->{LogObject}->Log( Priority => 'error', Message => "Called ----> ADAPTER_INTERFACE_4 <----" );
         $Self->SendHttpNotification (
                  SRID           =>  $Ticket{TicketNumber},
                  Action         =>  "Planned",
                  UserID         =>      $Param{UserID},
...
sub StateSet {
	...
	#CR1490 Aggiunto parametro per gestire la fine della chiusura per i ticket customer gestiti dagli agent
             ...
             if (defined($Ticket{CustomerUserID})){
             	$MS_adapter_OTRS_Interface_chiamato = 1;
             	$Self->{LogObject}->Log( Priority => 'error', Message => "Called ----> ADAPTER_INTERFACE_5 <----" );
	             $Self->SendHttpNotification (
	                    SRID		=>	$Ticket{TicketNumber},
             	...
             	$MS_adapter_OTRS_Interface_chiamato = 1; #20140121: il workaround deve funzionare anche per PI
             	...
             	$MS_adapter_OTRS_Interface_chiamato = 1;
			 $Self->{LogObject}->Log( Priority => 'error', Message => "Called ----> ADAPTER_INTERFACE <----" );
                $Self->SendHttpNotification (
	                    SRID		=>	$Ticket{TicketNumber},
	                    Action		=>	"$actionField{$State{ID}}",
	                    NewID		=>	"$State{ID}",
	...
	)
	{
		...
		while(!$MS_ok or $MS_currentTime >= $MS_maxTime)
		{
			...
			{	
				...
				$MS_ok = 1 if($MS_Row[0] eq $MS_PM_Wind_settings_ptr->{ticketStateID_ChiusoRIS});
		...
		if($MS_ok)
		{
			...
			{	
				# aggiungo alla history questo cambio di stato "forzato"
				$Self->HistoryAdd(
					TicketID     => $Param{TicketID},
					ArticleID    => $ArticleID,
...
sub TicketAcl {
    ...
    for my $Acl ( sort keys %Acls ) {
		$Self->{LogObject}->Log ( Priority	=> 'error',  Message => "############## $Acl");
            ...
            )
        {
            ...
            # not possible list
            for my $ID ( keys %Data ) {
				#$Self->{LogObject}->Log ( Priority	=> 'error',  Message => "############## $Data{$ID}");
                my $Match = 1;
...
$Self->{LogObject}->Log( Priority => 'info', Message => "===> END END <===" );
