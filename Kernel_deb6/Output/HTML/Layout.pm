...
#use lib "$Bin/../../Kernel/pm_wind"; #MS: per i miei moduli custom
    ...
    my $MS_current_ticket_freetime1 = '';
    my $MS_current_ticket_freetime2 = '';
    my $MS_current_ticket_freetime3 = '';
    my $MS_current_ticket_vf_closing_code = '';
    my $MS_current_ticket_vf_error_message = '';
    my $MS_current_ticket_typeID = '';
    my $MS_current_ticket_freetext15 = '';
    my $MS_current_ticket_freetext16 = '';
    my $MS_current_ticket_Valid_Article_for_Wind = '-1'; # -1 = no,  >0 = si 
    
    my $MS_current_ticket_WIND_FREETEXT2_SPLITTED = '';
    my $MS_current_ticket_WIND_MSISDN = '';
    my $MS_current_ticket_WIND_ID_LINEA = '';
    
    my $MS_current_ticket_WIND_FREETEXT3_SPLITTED = '';
    my $MS_current_ticket_WIND_IMSI = '';
    my $MS_current_ticket_WIND_ICCID = '';
    my $MS_current_ticket_WIND_IMEI = '';
    
    my $MS_current_ticket_WIND_FREETEXT10_SPLITTED = '';
    my $MS_current_ticket_WIND_AMBITOTT = '';
    my $MS_current_ticket_WIND_TIPO_LINEA = '';
    my $MS_current_ticket_WIND_MNP_TYPE = '';
    
    my $MS_current_ticket_WIND_FREETEXT11_SPLITTED = '';
    my $MS_current_ticket_WIND_Comune = '';
    my $MS_current_ticket_WIND_Provincia = '';
    my $MS_current_ticket_WIND_Indirizzo = '';
    my $MS_current_ticket_WIND_CAP = '';
  

    
    
    if(exists($Param{Data}->{TicketID}) and defined($Param{Data}->{TicketID}) and $Param{Data}->{TicketID} =~ m/^\d+$/ and $Param{Data}->{TicketID} > 0)
    {
        my $MS_TicketID = $Param{Data}->{TicketID};
        
        my $MS_tmp_DBObject = Kernel::System::DB->new(
              ConfigObject => $Self->{ConfigObject},
              EncodeObject => $Self->{EncodeObject},
              LogObject    => $Self->{LogObject},
              MainObject   => $Self->{MainObject},
        ...
        my $MS_query = 'select freetime1, freetime2, freetime2, vf_closing_code, vf_error_message, freetext2, freetext3, freetext10, freetext11, type_id, freetext15, freetext16 from ticket where id = '.$MS_TicketID;
        
        $MS_tmp_DBObject->Prepare(
             SQL   => $MS_query,
             Limit => 1
         );
         my @MS_Row = $MS_tmp_DBObject->FetchrowArray();
         
        if (scalar(@MS_Row) > 0)
        {			
           $MS_current_ticket_freetime1 = $MS_Row[0];
           $MS_current_ticket_freetime2 = $MS_Row[1];
           $MS_current_ticket_freetime3 = $MS_Row[2];
           $MS_current_ticket_vf_closing_code = $MS_Row[3];
           $MS_current_ticket_vf_error_message = $MS_Row[4];
           $MS_current_ticket_typeID = $MS_Row[9];
           $MS_current_ticket_freetext15 = $MS_Row[10];
           $MS_current_ticket_freetext16 = $MS_Row[11];
           
           my $tmpHash = {};
           $MS_current_ticket_Valid_Article_for_Wind = MSTicketUtil::MS_CheckIfExistsFreshArticleForWind($Param{Data}->{TicketID}, $MS_tmp_DBObject, $tmpHash);

           
           my $TicketHash_ptr = {};
           $TicketHash_ptr->{FREETEXT2} = $MS_Row[5];
           $TicketHash_ptr->{FREETEXT3} = $MS_Row[6];
           $TicketHash_ptr->{FREETEXT10} = $MS_Row[7];
           $TicketHash_ptr->{FREETEXT11} = $MS_Row[8];
           
           MS_SplitMergedFields($TicketHash_ptr);
   

            $MS_current_ticket_WIND_FREETEXT2_SPLITTED = $TicketHash_ptr->{WIND_FREETEXT2_SPLITTED};
            $MS_current_ticket_WIND_MSISDN = $TicketHash_ptr->{WIND_MSISDN};
            $MS_current_ticket_WIND_ID_LINEA = $TicketHash_ptr->{WIND_ID_LINEA};
                
            $MS_current_ticket_WIND_FREETEXT3_SPLITTED = $TicketHash_ptr->{WIND_FREETEXT3_SPLITTED};
            $MS_current_ticket_WIND_IMSI = $TicketHash_ptr->{WIND_IMSI};
            $MS_current_ticket_WIND_ICCID = $TicketHash_ptr->{WIND_ICCID};
            $MS_current_ticket_WIND_IMEI = $TicketHash_ptr->{WIND_IMEI};
                
            $MS_current_ticket_WIND_FREETEXT10_SPLITTED = $TicketHash_ptr->{WIND_FREETEXT10_SPLITTED};
            $MS_current_ticket_WIND_AMBITOTT = $TicketHash_ptr->{WIND_AMBITOTT};
            $MS_current_ticket_WIND_TIPO_LINEA = $TicketHash_ptr->{WIND_TIPO_LINEA};	
            $MS_current_ticket_WIND_MNP_TYPE = $TicketHash_ptr->{WIND_MNP_TYPE};
                
            $MS_current_ticket_WIND_FREETEXT11_SPLITTED = $TicketHash_ptr->{WIND_FREETEXT11_SPLITTED};
            $MS_current_ticket_WIND_Comune = $TicketHash_ptr->{WIND_Comune};
            $MS_current_ticket_WIND_Provincia = $TicketHash_ptr->{WIND_Provincia};
            $MS_current_ticket_WIND_Indirizzo = $TicketHash_ptr->{WIND_Indirizzo};
            $MS_current_ticket_WIND_CAP = $TicketHash_ptr->{WIND_CAP};
            
            
            
            

            #esporto valori specifici del ticket corrente
            $Self->{EnvRef}->{MS_current_ticket_freetime1} = $MS_current_ticket_freetime1;
            $Self->{EnvRef}->{MS_current_ticket_freetime2} = $MS_current_ticket_freetime2;
            $Self->{EnvRef}->{MS_current_ticket_freetime3} = $MS_current_ticket_freetime3;
            $Self->{EnvRef}->{MS_current_ticket_vf_closing_code} = $MS_current_ticket_vf_closing_code;
            $Self->{EnvRef}->{MS_current_ticket_vf_error_message} = $MS_current_ticket_vf_error_message; #per Wind popolo con 'MS_full'
            $Self->{EnvRef}->{MS_current_ticket_typeID} = $MS_current_ticket_typeID;
            $Self->{EnvRef}->{MS_current_ticket_freetext15} = $MS_current_ticket_freetext15;
            $Self->{EnvRef}->{MS_current_ticket_freetext16} = $MS_current_ticket_freetext16;
            
            $Self->{EnvRef}->{MS_current_ticket_Valid_Article_for_Wind} = $MS_current_ticket_Valid_Article_for_Wind;
        
            
            $Self->{EnvRef}->{MS_current_ticket_WIND_FREETEXT2_SPLITTED} =  $MS_current_ticket_WIND_FREETEXT2_SPLITTED;
            $Self->{EnvRef}->{MS_current_ticket_WIND_MSISDN} =  $MS_current_ticket_WIND_MSISDN;
            $Self->{EnvRef}->{MS_current_ticket_WIND_ID_LINEA} =  $MS_current_ticket_WIND_ID_LINEA;
            
            $Self->{EnvRef}->{MS_current_ticket_WIND_FREETEXT3_SPLITTED} =  $MS_current_ticket_WIND_FREETEXT3_SPLITTED;
            $Self->{EnvRef}->{MS_current_ticket_WIND_IMSI} =  $MS_current_ticket_WIND_IMSI;
            $Self->{EnvRef}->{MS_current_ticket_WIND_ICCID} =  $MS_current_ticket_WIND_ICCID;
            $Self->{EnvRef}->{MS_current_ticket_WIND_IMEI} =  $MS_current_ticket_WIND_IMEI;
            
            $Self->{EnvRef}->{MS_current_ticket_WIND_FREETEXT10_SPLITTED} =  $MS_current_ticket_WIND_FREETEXT10_SPLITTED;
            $Self->{EnvRef}->{MS_current_ticket_WIND_AMBITOTT} =  $MS_current_ticket_WIND_AMBITOTT;
            $Self->{EnvRef}->{MS_current_ticket_WIND_TIPO_LINEA} =  $MS_current_ticket_WIND_TIPO_LINEA;
            $Self->{EnvRef}->{MS_current_ticket_WIND_MNP_TYPE} =  $MS_current_ticket_WIND_MNP_TYPE;
            
            $Self->{EnvRef}->{MS_current_ticket_WIND_FREETEXT11_SPLITTED} =  $MS_current_ticket_WIND_FREETEXT11_SPLITTED;
            $Self->{EnvRef}->{MS_current_ticket_WIND_Comune} =  $MS_current_ticket_WIND_Comune;
            $Self->{EnvRef}->{MS_current_ticket_WIND_Provincia} =  $MS_current_ticket_WIND_Provincia;
            $Self->{EnvRef}->{MS_current_ticket_WIND_Indirizzo} =  $MS_current_ticket_WIND_Indirizzo;
            $Self->{EnvRef}->{MS_current_ticket_WIND_CAP} =  $MS_current_ticket_WIND_CAP;
        }
    }
	...
	my %MS_PM_Wind_settings = %{ $Self->{ConfigObject}->Get( 'PM_Wind_settings' )};
	my %MS_Category_Incident_PM_Wind = %{ $Self->{ConfigObject}->Get( 'Category_Incident_PM_Wind' )};
	my %MS_Category_Alarm_PM_Wind = %{ $Self->{ConfigObject}->Get( 'Category_Alarm_PM_Wind' )};
	my %MS_Category_Alarm_Wind_PM = %{ $Self->{ConfigObject}->Get( 'Category_Alarm_Wind_PM' )};
	my %MS_AmbitoTT_PM_Wind = %{ $Self->{ConfigObject}->Get( 'AmbitoTT_PM_Wind' )};
   my %MS_TipoLinea_PM_Wind = %{ $Self->{ConfigObject}->Get( 'TipoLinea_PM_Wind' )};
   my %MS_MNPType_PM_Wind = %{ $Self->{ConfigObject}->Get( 'MNPType_PM_Wind' )};
   my %MS_Marcaggio_PM_Wind = %{ $Self->{ConfigObject}->Get( 'Marcaggio_PM_Wind' )};
	
	my $MS_tmp_string = '';
	my $MS_sep = '#!#';
	
	
	foreach my $key (keys %MS_Category_Incident_PM_Wind)
	{
		$MS_tmp_string .= $MS_Category_Incident_PM_Wind{$key}.$MS_sep;
	}
	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
	$Self->{EnvRef}->{MS_Category_Incident_PM_Wind} = $MS_tmp_string;
   
   
	$MS_tmp_string = '';
	foreach my $key (keys %MS_Category_Alarm_PM_Wind)
	{
		$MS_tmp_string .= $MS_Category_Alarm_PM_Wind{$key}.$MS_sep;
	}
	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
	$Self->{EnvRef}->{MS_Category_Alarm_PM_Wind} = $MS_tmp_string;
  
  
	$MS_tmp_string = '';
	foreach my $key (keys %MS_Category_Alarm_Wind_PM)
	{
		$MS_tmp_string .= $MS_Category_Alarm_Wind_PM{$key}.$MS_sep;
	}
	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
	$Self->{EnvRef}->{MS_Category_Alarm_Wind_PM} = $MS_tmp_string;
   
   
	$MS_tmp_string = '';
	foreach my $key (keys %MS_AmbitoTT_PM_Wind)
	{
		$MS_tmp_string .= $MS_AmbitoTT_PM_Wind{$key}.$MS_sep;
	}
	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
	$Self->{EnvRef}->{MS_AmbitoTT_PM_Wind} = $MS_tmp_string;
 
 
 	$MS_tmp_string = '';
	foreach my $key (keys %MS_TipoLinea_PM_Wind)
	{
		$MS_tmp_string .= $MS_TipoLinea_PM_Wind{$key}.$MS_sep;
	}
	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
	$Self->{EnvRef}->{MS_TipoLinea_PM_Wind} = $MS_tmp_string;
 
 
  	$MS_tmp_string = '';
	foreach my $key (keys %MS_MNPType_PM_Wind)
	{
		$MS_tmp_string .= $MS_MNPType_PM_Wind{$key}.$MS_sep;
	}
	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
	$Self->{EnvRef}->{MS_MNPType_PM_Wind} = $MS_tmp_string;
 
 
  	$MS_tmp_string = '';
	foreach my $key (keys %MS_Marcaggio_PM_Wind)
	{
		$MS_tmp_string .= $MS_Marcaggio_PM_Wind{$key}.$MS_sep;
	}
	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
	$Self->{EnvRef}->{MS_Marcaggio_PM_Wind} = $MS_tmp_string;
   
   
   # i nuovi stati
   $Self->{EnvRef}->{MS_ticketStateID_Sospeso} = $MS_PM_Wind_settings{ticketStateID_Sospeso};
   $Self->{EnvRef}->{MS_ticketStateID_RisoltoNRI} = $MS_PM_Wind_settings{ticketStateID_RisoltoNRI};
   $Self->{EnvRef}->{MS_ticketStateID_RisoltoNoACT} = $MS_PM_Wind_settings{ticketStateID_RisoltoNoACT};
   

   # i nuovi tipi di ticket
    $Self->{EnvRef}->{MS_ticketTypeID_IncidentToWind} = $MS_PM_Wind_settings{ticketTypeID_IncidentToWind};
    $Self->{EnvRef}->{MS_ticketTypeID_IncidentToPM} = $MS_PM_Wind_settings{ticketTypeID_IncidentToPM};
    $Self->{EnvRef}->{MS_ticketTypeID_IncidentToEricsson} = $MS_PM_Wind_settings{ticketTypeID_IncidentToEricsson};
    $Self->{EnvRef}->{MS_ticketTypeID_AlarmFromWind} = $MS_PM_Wind_settings{ticketTypeID_AlarmFromWind};
    $Self->{EnvRef}->{MS_ticketTypeID_AlarmToWind} = $MS_PM_Wind_settings{ticketTypeID_AlarmToWind};
    $Self->{EnvRef}->{MS_ticketTypeID_AlarmToEricsson} = $MS_PM_Wind_settings{ticketTypeID_AlarmToEricsson};
    $Self->{EnvRef}->{MS_ticketTypeID_AlarmToPM} = $MS_PM_Wind_settings{ticketTypeID_AlarmToPM};

    # le nuove code
    $Self->{EnvRef}->{MS_queue_id_incident_for_wind} = $MS_PM_Wind_settings{queue_id_incident_for_wind};
    $Self->{EnvRef}->{MS_queue_id_alarm_to_wind} = $MS_PM_Wind_settings{queue_id_alarm_to_wind};
    $Self->{EnvRef}->{MS_queue_id_alarm_from_wind} = $MS_PM_Wind_settings{queue_id_alarm_from_wind};
    $Self->{EnvRef}->{MS_queue_id_ericsson} = $MS_PM_Wind_settings{queue_id_ericsson};
    $Self->{EnvRef}->{'MS_queue_id_MVNE-FE-Wind-Ericsson-Alarm'} = $MS_PM_Wind_settings{'queue_id_MVNE-FE-Wind-Ericsson-Alarm'};
    $Self->{EnvRef}->{MS_queue_id_ERICSSON_PM_WIND_OUT} = $MS_PM_Wind_settings{queue_id_ERICSSON_PM_WIND_OUT};
    $Self->{EnvRef}->{'MS_queue_id_MVNE-FE-Ericsson-Incident'} = $MS_PM_Wind_settings{'queue_id_MVNE-FE-Ericsson-Incident'};
    $Self->{EnvRef}->{'MS_queue_id_ERICSSON-WIND-OUT-Alarm'} = $MS_PM_Wind_settings{'queue_id_ERICSSON-WIND-OUT-Alarm'};
    $Self->{EnvRef}->{'MS_queue_id_WIND-ERICSSON-OUT-Alarm'} = $MS_PM_Wind_settings{'queue_id_WIND-ERICSSON-OUT-Alarm'};
    $Self->{EnvRef}->{'MS_queue_id_ERICSSON-OUT-Alarm'} = $MS_PM_Wind_settings{'queue_id_ERICSSON-OUT-Alarm'};
    $Self->{EnvRef}->{'MS_queue_id_MVNE-FE-Ericsson-Alarm'} = $MS_PM_Wind_settings{'queue_id_MVNE-FE-Ericsson-Alarm'};
    $Self->{EnvRef}->{'MS_queue_id_PM-Create'} = $MS_PM_Wind_settings{'queue_id_PM-Create'};
    $Self->{EnvRef}->{'MS_queue_id_ERICSSON-Create'} = $MS_PM_Wind_settings{'queue_id_ERICSSON-Create'};
  
    # tempo di validita' nota per Wind
    $Self->{EnvRef}->{MS_article_validityTime_ToWind} = $MS_PM_Wind_settings{article_validityTime_ToWind};  
...
#          );
#
#        my $MS_query = 'select freetime1, freetime2, freetime2, vf_closing_code, vf_error_message, freetext2, freetext3, freetext10, freetext11, type_id from ticket where id = '.$MS_TicketID;
#        
#        $MS_tmp_DBObject->Prepare(
#             SQL   => $MS_query,
#             Limit => 1
...
#         
#         
#        #my ($ms_package, $ms_filename, $ms_line) = caller;
#        #$Self->{LogObject}->Log(
#        #    Priority => 'error',
...
#
#            #esporto valori specifici del ticket corrente
#            $GlobalRef->{Env}->{MS_current_ticket_freetime1} = $MS_current_ticket_freetime1;
#            $GlobalRef->{Env}->{MS_current_ticket_freetime2} = $MS_current_ticket_freetime2;
#            $GlobalRef->{Env}->{MS_current_ticket_freetime3} = $MS_current_ticket_freetime3;
#            $GlobalRef->{Env}->{MS_current_ticket_vf_closing_code} = $MS_current_ticket_vf_closing_code;
#            $GlobalRef->{Env}->{MS_current_ticket_vf_error_message} = $MS_current_ticket_vf_error_message; #per Wind popolo con 'MS_full'
#            $GlobalRef->{Env}->{MS_current_ticket_typeID} = $MS_current_ticket_typeID;
#            #$Param{Data}->{MS_current_ticket_typeID} = $MS_current_ticket_typeID;
#            ##SetEnv( Key => 'MS_ZZZ', Value => $MS_current_ticket_typeID );
#            #$Self->{EnvNewRef}->{MS_ZZZ} = $MS_current_ticket_typeID;
#            #
#            #     $Self->{LogObject}->Log(
#            #        Priority => 'error',
#            #        Message  => '***************!!!!!zzzzzzzz!!!!!**************'."\n".$MS_current_ticket_typeID,
...
#            #     $Self->{LogObject}->Log(
#            #        Priority => 'error',
#            #        Message  => '***************!!!!!qqqqqqqqqq!!!!!**************'."\n".$GlobalRef->{Env}->{MS_current_ticket_typeID},
#            #    );   
#            
#            
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_FREETEXT2_SPLITTED} =  $MS_current_ticket_WIND_FREETEXT2_SPLITTED;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_MSISDN} =  $MS_current_ticket_WIND_MSISDN;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_ID_LINEA} =  $MS_current_ticket_WIND_ID_LINEA;
#            
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_FREETEXT3_SPLITTED} =  $MS_current_ticket_WIND_FREETEXT3_SPLITTED;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_IMSI} =  $MS_current_ticket_WIND_IMSI;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_ICCID} =  $MS_current_ticket_WIND_ICCID;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_IMEI} =  $MS_current_ticket_WIND_IMEI;
#            
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_FREETEXT10_SPLITTED} =  $MS_current_ticket_WIND_FREETEXT10_SPLITTED;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_AMBITOTT} =  $MS_current_ticket_WIND_AMBITOTT;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_TIPO_LINEA} =  $MS_current_ticket_WIND_TIPO_LINEA;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_MNP_TYPE} =  $MS_current_ticket_WIND_MNP_TYPE;
#            
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_FREETEXT11_SPLITTED} =  $MS_current_ticket_WIND_FREETEXT11_SPLITTED;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_Comune} =  $MS_current_ticket_WIND_Comune;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_Provincia} =  $MS_current_ticket_WIND_Provincia;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_Indirizzo} =  $MS_current_ticket_WIND_Indirizzo;
#            $GlobalRef->{Env}->{MS_current_ticket_WIND_CAP} =  $MS_current_ticket_WIND_CAP;
#            
#            #if ($Output !~ m/XXXXXXXXXXXXXXXX/)
#            #{
...
#	}
#	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
#	$GlobalRef->{Env}->{MS_Category_Incident_PM_Wind} = $MS_tmp_string;
#   
#   
#	$MS_tmp_string = '';
#	foreach my $key (keys %MS_Category_Alarm_PM_Wind)
...
#	}
#	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
#	$GlobalRef->{Env}->{MS_Category_Alarm_PM_Wind} = $MS_tmp_string;
#  
#  
#	$MS_tmp_string = '';
#	foreach my $key (keys %MS_Category_Alarm_Wind_PM)
...
#	}
#	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
#	$GlobalRef->{Env}->{MS_Category_Alarm_Wind_PM} = $MS_tmp_string;
#   
#   
#	$MS_tmp_string = '';
#	foreach my $key (keys %MS_AmbitoTT_PM_Wind)
...
#	}
#	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
#	$GlobalRef->{Env}->{MS_AmbitoTT_PM_Wind} = $MS_tmp_string;
# 
# 
# 	$MS_tmp_string = '';
#	foreach my $key (keys %MS_TipoLinea_PM_Wind)
...
#	}
#	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
#	$GlobalRef->{Env}->{MS_TipoLinea_PM_Wind} = $MS_tmp_string;
# 
# 
#  	$MS_tmp_string = '';
#	foreach my $key (keys %MS_MNPType_PM_Wind)
...
#	}
#	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
#	$GlobalRef->{Env}->{MS_MNPType_PM_Wind} = $MS_tmp_string;
# 
# 
#  	$MS_tmp_string = '';
#	foreach my $key (keys %MS_Marcaggio_PM_Wind)
...
#	}
#	$MS_tmp_string =~ s/$MS_sep$// if($MS_tmp_string ne '');
#	$GlobalRef->{Env}->{MS_Marcaggio_PM_Wind} = $MS_tmp_string;
#   
#   
#   # i nuovi stati
#   $GlobalRef->{Env}->{MS_ticketStateID_Sospeso} = $MS_PM_Wind_settings{ticketStateID_Sospeso};
#   $GlobalRef->{Env}->{MS_ticketStateID_RisoltoNRI} = $MS_PM_Wind_settings{ticketStateID_RisoltoNRI};
#   $GlobalRef->{Env}->{MS_ticketStateID_RisoltoNoACT} = $MS_PM_Wind_settings{ticketStateID_RisoltoNoACT};
#   
#
#   # i nuovi tipi di ticket
#    $GlobalRef->{Env}->{MS_ticketTypeID_IncidentToWind} = $MS_PM_Wind_settings{ticketTypeID_IncidentToWind};
#    $GlobalRef->{Env}->{MS_ticketTypeID_IncidentToPM} = $MS_PM_Wind_settings{ticketTypeID_IncidentToPM};
#    $GlobalRef->{Env}->{MS_ticketTypeID_IncidentToEricsson} = $MS_PM_Wind_settings{ticketTypeID_IncidentToEricsson};
#    $GlobalRef->{Env}->{MS_ticketTypeID_AlarmFromWind} = $MS_PM_Wind_settings{ticketTypeID_AlarmFromWind};
#    $GlobalRef->{Env}->{MS_ticketTypeID_AlarmToWind} = $MS_PM_Wind_settings{ticketTypeID_AlarmToWind};
#    $GlobalRef->{Env}->{MS_ticketTypeID_AlarmToEricsson} = $MS_PM_Wind_settings{ticketTypeID_AlarmToEricsson};
#    $GlobalRef->{Env}->{MS_ticketTypeID_AlarmToPM} = $MS_PM_Wind_settings{ticketTypeID_AlarmToPM};
#
#
#	
#	# -------------- MS Env ADD (fine) ----------------
