...
sub Run {
  ...
  #  }
    ...
    $Self->{LogObject}->Log( Priority => 'error', Message => "_MS_Full_ -> ACL -> checkResult = $checkResult");
    if($checkResult < 0)
    {
       my @NotPossibleQueueWind = ();
       push( @NotPossibleQueueWind,  'WIND_OUT');
       push( @NotPossibleQueueWind, 'ERICSSON_PM_WIND_OUT');
       
       push( @NotPossibleQueueWind,  'WIND_OUT_Alarm');
       push( @NotPossibleQueueWind,  'ERICSSON-WIND-OUT-Alarm');
...
#### MS TEST (inizio)
#	if ( $Ticket{Queue} ne 'MIACODA' )
#	{
#        push( @NotPossibleState, ('MioStato', 'closed successful', 'closed unsuccessful') );
#			  if ( $typePrefix eq 'Alarm' )
#			  {
#           push( @NotPossibleState, ('MioStato', 'pending reminder', 'pending auto close-') );
#			  }  
#	}
#	else #Sono su MIACODA
#	{
#        push( @NotPossibleState, ('closed successful', 'closed unsuccessful') );
#			  if ( $typePrefix eq 'Alarm' )
#			  {
...
### MS TEST (inizio)
	my @StatiNONPossibili; 
	my @StatiPossibili; 
	if ( $Ticket{Queue} ne 'MIACODA' )
	{
        push( @StatiNONPossibili, 'MioStato');
