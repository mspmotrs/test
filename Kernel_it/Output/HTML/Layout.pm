...
#use lib "$Bin/../../Kernel/pm_wind"; #MS: per i miei moduli custom
    ...
    my $MS_current_ticket_TTL_Expired = '0';
    ...
    my $MS_current_ticket_channel = '';
    my $MS_current_ticket_stateID = '';
    my $MS_current_ticket_Valid_Article_for_Wind = '-1'; # -1 = no,  >0 = si
    my $MS_current_ticket_Valid_Article_for_Wind_incoming_time = 0;
    ...
    if(exists($Param{Data}->{TicketID}) and defined($Param{Data}->{TicketID}) and $Param{Data}->{TicketID} =~ m/^\d+$/ and $Param{Data}->{TicketID} > 0)
    {
        ...
        my $MS_query = 'select freetime1, freetime2, freetime3, vf_closing_code, vf_error_message, freetext2, freetext3, freetext10, freetext11, type_id, freetext15, freetext16, channel, ticket_state_id from ticket where id = '.$MS_TicketID;
        ...
        {			
           ...
           $MS_current_ticket_TTL_Expired = MSTicketUtil::MS_Is_TTL_Expired($MS_current_ticket_freetime3);
           ...
           $MS_current_ticket_channel = $MS_Row[12];
           $MS_current_ticket_stateID = $MS_Row[13];
           ...
           $TicketHash_ptr->{FREETEXT11} = $MS_Row[8];
            ...
            #recupero l'incomingtime per la nota valida (se presente)
           if ($MS_current_ticket_Valid_Article_for_Wind > 0)
           {
                my $MS_query_art = 'select incoming_time from article where id = '.$MS_current_ticket_Valid_Article_for_Wind;
                
                $MS_tmp_DBObject->Prepare(
                     SQL   => $MS_query_art,
                     Limit => 1
                 );
                 my @MS_Row_art = $MS_tmp_DBObject->FetchrowArray();
                 
                if (scalar(@MS_Row_art) > 0)
                {		
                    $MS_current_ticket_Valid_Article_for_Wind_incoming_time = $MS_Row_art[0];
                }
           }
           ...
           MS_SplitMergedFields($TicketHash_ptr);
            ...
            $Self->{EnvRef}->{MS_current_ticket_TTL_Expired} = $MS_current_ticket_TTL_Expired;
            
            $Self->{EnvRef}->{MS_current_ticket_vf_closing_code} = $MS_current_ticket_vf_closing_code; #causale (arriva da Wind)
            ...
            $Self->{EnvRef}->{MS_current_ticket_channel} = $MS_current_ticket_channel;
            $Self->{EnvRef}->{MS_current_ticket_stateID} = $MS_current_ticket_stateID;
            
            
            $Self->{EnvRef}->{MS_current_ticket_Valid_Article_for_Wind} = $MS_current_ticket_Valid_Article_for_Wind;
            $Self->{EnvRef}->{MS_current_ticket_Valid_Article_for_Wind_incoming_time} = $MS_current_ticket_Valid_Article_for_Wind_incoming_time;
   ...
   $Self->{EnvRef}->{MS_ticketStateID_ChiusoNRI} = $MS_PM_Wind_settings{ticketStateID_ChiusoNRI};
   $Self->{EnvRef}->{MS_ticketStateID_ChiusoNoACT} = $MS_PM_Wind_settings{ticketStateID_ChiusoNoACT};
   
   #alcuni dei vecchi stati
    $Self->{EnvRef}->{MS_ticketStateID_Risolto} = $MS_PM_Wind_settings{ticketStateID_Risolto};
    $Self->{EnvRef}->{MS_ticketStateID_ChiusoRIS} = $MS_PM_Wind_settings{ticketStateID_ChiusoRIS};
