...
=item MoveListCustomer() 
Funzione per avere la lista delle liste su cui il customer può spostare il ticket
=cut

sub MoveListCustomer {
    my ( $Self, %Param ) = @_;
	...
	my $oldUserID = $Param{UserID};

#$Self->{LogObject}->Log(Priority => 'error',Message  => "UserID:$oldUserID");
#$Self->{LogObject}->Log(Priority => 'error',Message  => "CustomerID:".$Param{CustomerID});

	my %CustomerMoveDestinationQueue;	
	%CustomerMoveDestinationQueue = %{ $Self->{ConfigObject}->Get("Ticket::Acl::CustomerMoveDestinationQueue") };
	#Leggo le code configurate da sistema manualmente
	my %Queues;
	my %MoveQueues = {};
	my %AllQueues = {};
	
	if (exists $CustomerMoveDestinationQueue{$Param{CustomerID}}){			
		%MoveQueues = %{ $CustomerMoveDestinationQueue{$Param{CustomerID}} };
	}
	
	#Leggo tutte le code su cui il customer ha il permessi di rw o move_into
	%AllQueues = $Self->{QueueObject}->GetAllQueuesCustomer(%Param);

#foreach my $i (keys %AllQueues){$Self->{LogObject}->Log(Priority => 'error',Message  => 'AllQueues:'.$AllQueues{$i} );}

	if (%MoveQueues){
		foreach my $i (keys %MoveQueues){

#$Self->{LogObject}->Log(Priority => 'error',Message  => 'MoveQueues:'.$MoveQueues{$i} );

			if (exists $AllQueues{$i}){
				$Queues{$i} = $AllQueues{$i};
			}			
		}		
	}

#foreach my $i (keys %Queues){$Self->{LogObject}->Log(Priority => 'error',Message  => 'Queues:'.$Queues{$i} );}
...
#    }
	...
	delete $Param{UserID};#Il delete del parametro è necessario perchè altrimenti vengono interpretate anche le ACL destinate agli agent
    # workflow
    my $ACL = $Self->TicketAcl(
        %Param,
        ReturnType    => 'Ticket',
    ...
    $Param{UserID} = $oldUserID;
    return %Queues;
}

sub CustomerMoveTicket {
    my ( $Self, %Param ) = @_;
    ...
    # permission check
    my %MoveList = $Self->MoveListCustomer( %Param, Type => 'move' );
    ...
    ## PM-2011X2 - Notifica Coda Lavorazione START
	my %notifyConf = %{ $Self->{ConfigObject}->Get( 'workingQueue' )};
    if ($result) {
				$Self->AuditTicketAction (
						Action		=>	'MoreInfo',
						Message		=>	"Ticket ticket $Param{TicketID} has been moved in the queue $Queue by $Param{UserID}.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'OK',
				);
				...
				$Self->AuditTicketAction (
						Action		=>	'MoreInfo',
						Message		=>	"Queue move of ticketID $Param{TicketID} to queue $Queue by $Param{UserID} has failed.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'KO',
				);
    ...
    if( $notifyConf{$Queue} ){
        if ( !$Param{ParentUpd} || $Param{ParentUpd} ne '1' ) {
             $Self->SendHttpNotification (
                 SRID		=>	$Ticket{TicketNumber},
                 Action		=>	"MoreInfo",
                 UserID		=>	$Param{UserID},
                 Notify    =>  uri_escape($notifyConf{$Queue}),
             );
         }
    }
    ## PM-2011X2 - Notifica Coda Lavorazione END
...
sub MoveTicket {
    ...
    ## CR1686 - CREATE/GETINFO/REOPEN FROM OTRS TO PI (es 30=PosteItaliane-OUT)
    if ( $Param{QueueID} eq '30' || $Param{QueueID} eq '66' ){
    	 #CR1686 - Based on the Ticket Type will be Called OTRS_Adapter(Es. PosteItaliane) or OTRS_Interface(Es. Siebel)
	    $Self->SendHttpNotificationToAdapter (
	           SRID		  =>	$Ticket{TicketNumber},
	           UserID		=>	$Param{UserID},
	           Action   =>  "",      
	     );
...
$Self->{LogObject}->Log( Priority => 'error', Message => "BEFORE TicketEventHandlerPost..." );
...
$Self->{LogObject}->Log( Priority => 'error', Message => "AFTER TicketEventHandlerPost..." );
...
sub TicketTypeList {
	...
	$Self->{LogObject}->Log(Priority => 'error', Message => 'Calling ACL TicketTypeList');
    # workflow
    my $ACL = $Self->TicketAcl(
        %Param,
        ReturnType    => 'Ticket',
...
sub TicketTypeSet {
    ...
    if ($result) {
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Ticket type of ticket $Param{TicketID} set to state $Param{TypeID} by $Param{UserID}.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'OK',
				);
				...
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Type modification of ticketID $Param{TicketID} to type $Param{TypeID} by $Param{UserID} has failed.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'KO',
				);
...
sub TicketServiceSet {
    ...
    if ($result) {
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Service of ticket $Param{TicketID} set to $Param{ServiceID} by $Param{UserID}.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'OK',
				);
				...
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Service modification of ticketID $Param{TicketID} to service $Param{ServiceID} by $Param{UserID} has failed.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'KO',
				);
...
sub TicketSLASet {
    ...
    if ($result) {
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"SLA of ticket $Param{TicketID} set to $Param{SLAID} by $Param{UserID}.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'OK',
				);
				...
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"SLA modification of ticketID $Param{TicketID} to SLA $Param{SLAID} by $Param{UserID} has failed.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'KO',
				);
...
#########PM: custom Searching method
sub CustomTicketFreeTextGet {
    my ( $Self, %Param ) = @_;
...
#    my %var = %{ $Self->{ConfigObject}->Get( 'workingQueue' )};
#    for my $op ( keys %var  ){
#       $Self->{LogObject}->Log( Priority => 'error', Message => "CONSTANT::>$Param{ParentUpd}<:: $op . $var{$op}" );
#    }
#my %TicketFreeVarKey = %{ $Self->{ConfigObject}->Get( 'workingQueue' )};
#my @FreeVarKey          = keys %TicketFreeVarKey;
#$Self->{LogObject}->Log( Priority => 'error', Message => "CONSTANT::::  @FreeVarKey" );

$Self->{LogObject}->Log( Priority => 'error', Message => "Before Planned Date ::: $Ticket{TicketNumber} " );

    if( $Param{Counter} eq 4 ){
         my $updateDate = uri_escape($TimeStamp);
         $updateDate =~ s/\%20/\T/g;   
         $Self->AuditTicketAction (
                  Action         =>      'Planned',
                  Message        =>      "FreeTime4 (Planned Date) field $updateDate have been updated on TicketID $Self->{TicketID} by UserID $Self->{UserID}.",
                  UserID         =>      $Param{UserID},
                  TicketID       =>      $Param{TicketID},
                  Body           =>      'No',
                  Result         =>      'OK',
      ...
      if ( !$Param{ParentUpd} || $Param{ParentUpd} ne '1' ) { 
         $Self->SendHttpNotification (
                  ...
                  Action         =>  "Planned",
                  UserID         =>      $Param{UserID},
                  PlannedDate    =>      $updateDate,
         );
      } 
    }

$Self->{LogObject}->Log( Priority => 'error', Message => "After Planned Date ::: $Ticket{TicketNumber} " );
...
sub LockSet {
    ...
    if ( $Param{Lock} =~ /^unlock$/i && $Ticket{StateID} eq 10 ) {
        $Self->StateSet (
						State    => 'open',
						TicketID => $Param{TicketID},
						UserID   => $Param{UserID},
				);
    } elsif ( $Param{Lock} =~ /^lock$/i && $Ticket{StateID} eq 4)  {
        $Self->StateSet (
						State    => 'in progress',
						TicketID => $Param{TicketID},
						UserID   => $Param{UserID},
		...
		if ($result) {
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Ticket $Param{TicketID} has locked by $Param{UserID}.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'OK',
				);
				...
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Locking ticket $Param{TicketID} to $Param{UserID} by $Param{UserID} has failed.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'KO',
				);
...
sub StateSet {
	...
	# ------- MS full (inizio parte I) ------
    # state id lookup
    my %MS_State;
    #devo essere sicuro di avere l'ID dello stato...
    if ( !$Param{StateID} ) {
        %MS_State = $Self->{StateObject}->StateGet( Name => $Param{State} );
    }
    else
    {
    	$MS_State{ID} = $Param{StateID};
    }
    
	my $MS_orignal_StateID = $MS_State{ID}; #*******
	my $MS_state_changed = 0;
	
	my $MS_ticket_StateID = '';
	my $MS_ticket_TypeID = '';
	my $MS_ticket_Channel = '';
	my $MS_ticket_Vf_error_message = '';
	
	my $MS_adapter_OTRS_Interface_chiamato = 0;
	my $MS_PM_Wind_settings_ptr = $Self->{ConfigObject}->Get( 'PM_Wind_settings' );
	#my %MS_full_Ticket = $Self->TicketGet( TicketID => $Param{TicketID} );
	
	my $MS_full_query_1 = 'SELECT ticket_state_id, type_id, channel, vf_error_message from ticket WHERE id = '.$Param{TicketID};
	$Self->{DBObject}->Prepare(
		  SQL   => $MS_full_query_1,
		  Limit => 1
	 );
	my @MS_Row_1 = $Self->{DBObject}->FetchrowArray();
	 
	if (scalar(@MS_Row_1))
	{		
		$MS_ticket_StateID = $MS_Row_1[0];
		$MS_ticket_TypeID = $MS_Row_1[1];	
		$MS_ticket_Channel = $MS_Row_1[2];	
		$MS_ticket_Vf_error_message = $MS_Row_1[3];			
	}	
	
	#I nuovi stati di risoluzione sono selezionabili solo per SR ed Incident
	#ma io qui devo agire solo sulle SR del nostro ambito: metto temporaneamente in ticket in RISOLTO
	#e faccio fare al OTRS_Interface il suo lavoro
	if( ($MS_orignal_StateID eq $MS_PM_Wind_settings_ptr->{ticketStateID_RisoltoNRI} or 
			$MS_orignal_StateID eq $MS_PM_Wind_settings_ptr->{ticketStateID_RisoltoNoACT})
		and	
		$MS_ticket_TypeID ne $MS_PM_Wind_settings_ptr->{ticketTypeID_IncidentToWind}
		and
		$MS_ticket_TypeID ne $MS_PM_Wind_settings_ptr->{ticketTypeID_IncidentToPM}
		and
		$MS_ticket_TypeID ne $MS_PM_Wind_settings_ptr->{ticketTypeID_IncidentToEricsson}		
	)
	{
		#$Self->{DBObject}->Do(
		#				SQL => "UPDATE ticket SET ticket_state_id = ? WHERE id = ?",
		#				Bind => [ \$MS_PM_Wind_settings_ptr->{ticketStateID_Risolto}, \$Param{TicketID} ],
		#);	
		
		#Aggiorno SOLO il parametro in input... che rappresenta lo stato in cui il ticket sta' per essere portato
		#$Param{StateID} = $MS_PM_Wind_settings_ptr->{ticketStateID_Risolto};
      $Param{StateID} = $MS_PM_Wind_settings_ptr->{ticketStateID_NonDiCompetenza}; #Operation ha richiesto che vengano gestiti come "NonDiCompetenza" invece di "Risolto"
      
		$MS_state_changed = 1;
		
		#$Self->{LogObject}->Log( Priority => 'debug', Message => "!!!!!!!!!!!!!!!!!!!!!!!! -----> chiamato !!!!!!!!");
	}
	# ------- MS full (fine parte I) ------
    ...
    if ( ($State{Name} eq $Ticket{State}) || ( ($State{ID} eq 7 || $State{ID} eq 6 || $State{ID} eq 8) && $Param{ParentUpd} eq '1') ) {
    ...
    if ($result) {
            $Self->AuditTicketAction (
               Action		=>	'UPDATE',
               Message		=>	"Ticket ticket $Param{TicketID} changed to state $State{ID} by $Param{UserID}.",
               UserID		=>	$Param{UserID},
               TicketID	        =>	$Param{TicketID},
               Result		=>	'OK',
            );
            ...
            $Self->AuditTicketAction (
               Action		=>	'UPDATE',
               Message		=>	"State modification of ticketID $Param{TicketID} to state $State{ID} by $Param{UserID} has failed.",
               UserID		=>	$Param{UserID},
               TicketID	        =>	$Param{TicketID},
               Result		=>	'KO',
             );
    ...
    ###PM Modification: unlock the ticket if it has been closed or resolved
    ###PM-X2011 Added new state closed(3,11) and resolved(6,8) 
    if ( ($State{ID} eq 6 || $State{ID} eq 7 || $State{ID} eq 8 || $State{ID} eq 2 || $State{ID} eq 3 || $State{ID} eq 11) )  {
    		$Self->LockSet (
        		LockID   => 1,
        		TicketID => $Param{TicketID},
        		UserID   => $Param{UserID},
    ...
    if ( !$Param{ParentUpd} || $Param{ParentUpd} ne '1') {
         ## PM-2011X2 - Notifica Cambio Stato START
         my %actionField = %{ $Self->{ConfigObject}->Get( 'stateToAction' )};
         if( $actionField{$State{ID}} ){
	#CR1490 Aggiunto parametro per gestire la fine della chiusura per i ticket customer gestiti dagli agent
             
             my $urlOtrsInterface = $Self->{ConfigObject}->Get('Ticket::HTTPNotificationLink');
             my $urlAdapterPI     = $Self->{ConfigObject}->Get('Ticket::HTTPNotificationLinkToAdapter');
             
             my %typeToUrlField = %{ $Self->{ConfigObject}->Get( 'TypeToAdapter' )};
             my $extUrl = $typeToUrlField{$Ticket{TypeID}};
            

             #CR1686 - Call OTRS_Adapter se almeno un figlio del ticket ha channel = PosteItaliane - START 
             my $CallOTRSAdapter = 0;
             ...
             #$Self->{LogObject}->Log( Priority => 'debug', Message => "Ticket Children: @TicketIDs " );               
 
             # no slaves
             if ( @TicketIDs ) {
                 #$Self->{LogObject}->Log( Priority => 'debug', Message => "TicketID[0]: $TicketIDs[0] "); 
                 $Self->{DBObject}->Prepare(
                      SQL   => 'SELECT t.channel FROM ticket t WHERE id = ? AND t.channel = \'PosteItaliane\' ',
                      Bind  => [ \$TicketIDs[0] ],
                      Limit => 1,
                 );
             ...
             #$Self->{LogObject}->Log( Priority => 'debug', Message => "N.Children with Channel = PosteItaliane : $CallOTRSAdapter " );
             
             #CR1686 - Call OTRS_Adapter se almeno un figlio del ticket ha channel = PosteItaliane - END 
             
             if (defined($Ticket{CustomerUserID})){
             	
             	$MS_adapter_OTRS_Interface_chiamato = 1; # <------- MS full 
             	
	             $Self->SendHttpNotification (
	                    SRID		=>	$Ticket{TicketNumber},
	                    Action		=>	"$actionField{$State{ID}}",
	                    NewID		=>	"$State{ID}",
	                    NewDesc		=>	"$State{Name}",
	                    UserID		=>	$Param{UserID},
             ...
             } elsif ( $extUrl ne '' && index($urlAdapterPI, $extUrl) >= 0 ){
             
             	$MS_adapter_OTRS_Interface_chiamato = 1; #MS full (20140121): il workaround deve funzionare anche per PI
             
             	 #$Self->{LogObject}->Log( Priority => 'debug', Message => "Called PI Adapter $urlAdapterPI -- $extUrl " );  
                  #CR1686 - Based on the Ticket Type will be Called OTRS_Adapter(Es. PosteItaliane) or OTRS_Interface(Es. Siebel)
	             $Self->SendHttpNotificationToAdapter (
	                    SRID		  =>	$Ticket{TicketNumber},
	                    UserID		=>	$Param{UserID},
             ...
             } elsif ( $CallOTRSAdapter == 1 ){     
             
             	$MS_adapter_OTRS_Interface_chiamato = 1; #MS full (20140121): il workaround deve funzionare anche per PI
             	
             	 #$Self->{LogObject}->Log( Priority => 'debug', Message => "Called PI Adapter with child PI" );
	             $Self->SendHttpNotificationToAdapter (
	                    SRID		  =>	$Ticket{TicketNumber},
	                    UserID		=>	$Param{UserID},
	                    Action    =>  "$actionField{$State{ID}}",      
	...
	#Vecchio codice modificato
             ## PM-2011X2 - Notifica Cambio Stato END
             #$Self->SendHttpNotification (
             #       SRID			=>	$Ticket{TicketNumber},
             #       Action		=>	"$actionField{$State{ID}}",
             #       NewID			=>	"$State{ID}",
	...
	# ------- MS full (inizio parte II) ------
	#$Self->{LogObject}->Log( Priority => 'debug', Message => "!!!!!!!!!!!!!!!!!!!!!!!! --->  MS_adapter_OTRS_Interface_chiamato = $MS_adapter_OTRS_Interface_chiamato");
	if	(	$MS_adapter_OTRS_Interface_chiamato and $MS_state_changed and 
			($MS_orignal_StateID eq $MS_PM_Wind_settings_ptr->{ticketStateID_RisoltoNRI} or 
			$MS_orignal_StateID eq $MS_PM_Wind_settings_ptr->{ticketStateID_RisoltoNoACT} )
	)
	{
		#Controllo - eventualmente aspetto - che la OTRS_Interface abbia fatto il proprio lavoro
		my $MS_startTime = time();
		my $MS_currentTime = 0;
		my $MS_maxTime = 20; #aspetto al massimo X secondi
		my $MS_ok = 0;
		my $MS_util_Actual_QueueID = $Ticket{QueueID};
		while(!$MS_ok or $MS_currentTime >= $MS_maxTime)
		{
			#%MS_full_Ticket = $Self->TicketGet( TicketID => $Param{TicketID} );
			my $MS_full_query = 'SELECT ticket_state_id, queue_id from ticket WHERE id = '.$Param{TicketID};
			$Self->{DBObject}->Prepare(
				  SQL   => $MS_full_query,
				  Limit => 1
			 );
			my @MS_Row = $Self->{DBObject}->FetchrowArray();
			 
			if (scalar(@MS_Row))
			{	
				#$Self->{LogObject}->Log( Priority => 'debug', Message => "!!!!!!!!!!!!!!!!!!!!!!!! ---> MS_Row[0] = $MS_Row[0] ");
			
				#OTRS_Interface deve chiudere automaticamente il ticket
             if($MS_Row[0] eq $MS_PM_Wind_settings_ptr->{ticketStateID_ChiusoRIS} or
                $MS_Row[0] eq $MS_PM_Wind_settings_ptr->{ticketStateID_ChiusoNdC} or
                $MS_Row[0] eq $MS_PM_Wind_settings_ptr->{ticketStateID_ChiusoINF})
            {
                $MS_ok = 1;
            }
            
				$MS_util_Actual_QueueID = $MS_Row[1];			
			}
			
			sleep(1) if(!$MS_ok);			
			$MS_currentTime = time() - $MS_startTime;
		}
		
		#tutto ok o semplice timeout?
		if($MS_ok)
		{
			my $MS_next_StateID = -1;
			my $MS_prev_StateName = '';
			my $MS_next_StateName = '';
			if($MS_orignal_StateID eq $MS_PM_Wind_settings_ptr->{ticketStateID_RisoltoNRI})
			{
				$MS_next_StateID = $MS_PM_Wind_settings_ptr->{ticketStateID_ChiusoNRI};
				$MS_prev_StateName = 'RisoltoNRI';
				$MS_next_StateName = 'ChiusoNRI';
			}
			elsif($MS_orignal_StateID eq $MS_PM_Wind_settings_ptr->{ticketStateID_RisoltoNoACT})
			{
				$MS_next_StateID = $MS_PM_Wind_settings_ptr->{ticketStateID_ChiusoNoACT};
				$MS_prev_StateName = 'RisoltoNoACT';
				$MS_next_StateName = 'ChiusoNoACT';
			}
			
			if($MS_next_StateID > 0)
			{
            my $historyID_risoluzione = 0;
            my $historyID_chiusura = 0;
            
            #trovo il la riga dell'history con il passaggio in risoluzione
				$Self->{DBObject}->Prepare(
								SQL => "select id, name from ticket_history where id = (select MAX(id) from ticket_history where ticket_id = ? and history_type_id = 27 and (name LIKE '%pending auto close-\%\%' or name LIKE '%pending auto close+\%\%' or name LIKE '%pending reminder\%\%'))",
                        Bind => [ \$Param{TicketID} ],
                        Limit => 1  
				);	
            my @RowHistory_risoluzione = $Self->{DBObject}->FetchrowArray();
            $historyID_risoluzione = $RowHistory_risoluzione[0] if(scalar(@RowHistory_risoluzione) > 0);

            
            #trovo il la riga dell'history con il passaggio in chiusura
				$Self->{DBObject}->Prepare(
								SQL => "select id, name from ticket_history where id = (select MAX(id) from ticket_history where ticket_id = ? and history_type_id = 27 and (name LIKE '%closed successful\%\%' or name LIKE '%closed unsuccessful\%\%' or name LIKE '%closed unsuccessful inf\%\%'))",
                        Bind => [ \$Param{TicketID} ],
                        Limit => 1  
				);	
            my @RowHistory_chiusura = $Self->{DBObject}->FetchrowArray();
            $historyID_chiusura = $RowHistory_chiusura[0] if(scalar(@RowHistory_chiusura) > 0);

            
            #trovo il nome dello stato in cui era il ticket prima di questi passaggi
            my $MS_old_state_name = '';
				$Self->{DBObject}->Prepare(
								SQL => "select name from ticket_state where id = ?",
                        Bind => [ \$MS_ticket_StateID ],
                        Limit => 1  
				);	
            my @Row_findStateName = $Self->{DBObject}->FetchrowArray();
            $MS_old_state_name = $Row_findStateName[0] if(scalar(@Row_findStateName) > 0);
            
            
            ##aggiorno la riga dell'history per la risoluzione (non faccio la delete)
            my $tmp_string_name = '%%'.$MS_old_state_name.'%%'.$MS_prev_StateName.'%%';
            #aggiorno direttamente le righe di history che dovrei cancellare...
            if ($historyID_risoluzione > 0 and $MS_old_state_name !~ m/^\s*$/)
            {
                $Self->{DBObject}->Do(
                            SQL => "UPDATE ticket_history SET name = ? WHERE id = ? and ticket_id = ?",
                            Bind => [ \$tmp_string_name, \$historyID_risoluzione, \$Param{TicketID} ],
                );
            }
            

            ##aggiorno la riga dell'history per la chiusura (non faccio la delete)
            $tmp_string_name = '%%'.%$MS_prev_StateName.'%%'.%$MS_next_StateName.'%%';
            #aggiorno direttamente le righe di history che dovrei cancellare...
            if ($historyID_risoluzione > 0 and $MS_old_state_name !~ m/^\s*$/)
            {
                $Self->{DBObject}->Do(
				...
				## aggiungo alla history quello che doveva essere il passaggio da "risolto xxx" a "chiuso xxx" ma che gli adapter Java non sanno fare
				#$Self->HistoryAdd(
				#	TicketID     => $Param{TicketID},
				#	ArticleID    => $ArticleID,
				#	QueueID      => $MS_util_Actual_QueueID,
				#	#Name         => "\%\%closed successful\%\%$MS_next_StateName\%\%",
				#	Name         => "\%\%$MS_prev_StateName\%\%$MS_next_StateName\%\%",
				#	HistoryTypeID  => 27, # StateUpdate
				#	#HistoryType  => 'StateUpdate',
				#	CreateUserID => $Param{UserID},
				#);	
				...
				$Self->{DBObject}->Do(
								SQL => "UPDATE ticket SET ticket_state_id = ? WHERE id = ?",
								Bind => [ \$MS_next_StateID, \$Param{TicketID} ],
				);	
			}		
		}
	}
	# ------- MS full (fine parte II) ------
...
sub OwnerSet {
    ...
    if ($result) {
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Owner of ticketID $Param{TicketID} has been change to $Param{NewUserID} by $Param{UserID}.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'OK',
				);
				...
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Changing owner of ticketID $Param{TicketID} to $Param{NewUserID} by $Param{UserID} has failed.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'KO',
				);
...
sub ResponsibleSet {
    ...
    if ($result) {
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Responsible user $Param{NewUserID} assigned to ticketID $Param{TicketID} by $Param{UserID}.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'OK',
				);
				...
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Changing responsible of ticketID $Param{TicketID} to $Param{NewUserID} by $Param{UserID} has failed.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'KO',
				);
...
sub PrioritySet {
    ...
    if ($result) {
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Priority of ticketID $Param{TicketID} has been set to $Param{PriorityID} by $Param{UserID}.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'OK',
				);
				...
				$Self->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Changing priority of ticketID $Param{TicketID} to $Param{NewUserID} by $Param{UserID} has failed.",
						UserID		=>	$Param{UserID},
						TicketID	=>	$Param{TicketID},
						Result		=>	'KO',
				);
...
sub AuditTicketAction {
    my ( $Self, %Param ) = @_;

    # check needed stuff
    for (qw(TicketID UserID Action Message)) {
        if ( !$Param{$_} ) {
            $Self->{LogObject}->Log( Priority => 'error', Message => "Need $_!" );
            return;
        }
    ...
    my %UserData = $Self->{UserObject}->GetUserData(
		...
		#Retrieving profile
		my $profile = '';
		my @GroupData = $Self->{GroupObject}->GetRoleListByUser(
														UserID => $Param{UserID},
														Result => 'Name',
										);
		...
		if (length $profile > 1) {
				$profile = substr($profile,0,(length $profile)-1);
		}
		...
		if (!$Body ) {
				#get Article
	    	my @ArticleArray = $Self->ArticleGet(
    	   		TicketID    => $Param{TicketID},
        		ArticleType => [ 'email-external', 'phone' ],
        		UserID      => $Param{UserID},
  	 		);

				my $Article = $ArticleArray[0];
				$Body = $Article->{Body}||'Unretrieved';
		}

		$Body = $Self->{HTMLUtilsObject}->ToAscii(
         String => $Body,
    );
    ...
    #PM: audit log
 		$Self->{LogObject}->Log (
    		Priority	=> 'audit',
    		Message		=> $Param{Message} . " - TicketNumber=$Ticket{TicketNumber}",
        SessionID => '',
        User			=> $UserData{UserLogin},
        Profile		=> $profile,
        Action		=> $Param{Action},
        TicketId	=> $Ticket{TicketNumber},
        Parameter	=> $Ticket{Title}. '>>' . $Body,
...
sub SendHttpNotification {
    my ( $Self, %Param ) = @_;

		# check needed stuff
		for (qw(Action SRID UserID)) {
				if ( !$Param{$_} ) {
						$Self->{LogObject}->Log( Priority => 'error', Message => "Need $_!" );
						return;
				}
		...
		my $url= $Self->{ConfigObject}->Get('Ticket::HTTPNotificationLink');

		if (!$url || $url eq '' ) {
				$Self->{LogObject}->Log(
						Priority	=> 'info',
						Message		=> "Config Item Ticket::HTTPNotificationLink not retrieved; no notification has performed.",
		    );
		    return;
		}
    ...
    use LWP::UserAgent;
		...
		$url .= '?' . 'SRID=' . $Param{SRID} . '&' . 'Action=' . $Param{Action}. '&' . 'UserID=' . $Param{UserID};

 		for my $Key ( keys %Param ) {
        if ($Key eq 'SRID' || $Key eq 'Action' || $Key eq 'UserID') {
        		next;
        } else {
        		$url .= '&' . "$Key=" . $Param{$Key};
    		}
    ...
    if ($response->is_success) {
		    $Self->{LogObject}->Log(
						Priority	=> 'info',
						Message		=> "Action $Param{Action} of ticket $Param{SRID} successfully notified to $url.",
		    );
    } else {
		    $Self->{LogObject}->Log(
						Priority	=> 'info',
						Message		=> "Action $Param{Action} for ticket $Param{SRID} failed notification to $url. Cause: " . "$response->error_as_string",
		    );
    }
    my %Ticket = $Self->TicketGet(
               TicketID => $Param{TicketID},
...
sub SendHttpNotificationToAdapter {
    my ( $Self, %Param ) = @_;
		...
		my $url= $Self->{ConfigObject}->Get('Ticket::HTTPNotificationLinkToAdapter');

		if (!$url || $url eq '' ) {
				$Self->{LogObject}->Log(
						Priority	=> 'info',
						Message		=> "Config Item Ticket::HTTPNotificationLinkToAdapter not retrieved; no notification has performed.",
		    );
		    return;
		}
    ...
    else
    {
         if ($response->is_success) {
		        $Self->{LogObject}->Log(
						  Priority	=> 'info',
						  Message		=> "Ticket $Param{SRID} successfully notified to $url in order to CREATE/GETINFO/REOPEN.",
		        );
         }
         else {
		        $Self->{LogObject}->Log(
						  Priority	=> 'info',
						  Message		=> "Ticket $Param{SRID} failed notification to $url. Cause: " . "$response->error_as_string",
		        );
         }
    }
    my %Ticket = $Self->TicketGet(
