...
sub Run {
    ...
    if ( !$Access ) {
				$Self->{TicketObject}->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Impossible to add note TicketID $Self->{TicketID}: no $Self->{Config}->{Permission} permissions is granted for the UserID $Self->{UserID}.",
						UserID		=>	$Self->{UserID},
						TicketID	=>	$Self->{TicketID},
						Body			=>	'No',
						Result		=>	'KO',
        ...
        if ( !$Self->{TicketObject}->LockIsTicketLocked( TicketID => $Self->{TicketID} ) ) {
            ...
            if ( !$AccessOk ) {
                my $Output = $Self->{LayoutObject}->Header( Value => $Ticket{Number} );
								$Self->{TicketObject}->AuditTicketAction (
										Action		=>	'UPDATE',
										Message		=>	"Adding note action failed because userID $Self->{UserID} is not the owner of the ticket $Self->{TicketID}.",
										UserID		=>	$Self->{UserID},
										TicketID	=>	$Self->{TicketID},
										Body			=>	'No',
										Result		=>	'KO',
    ...
    if ( $Self->{Subaction} eq 'Store' ) {
        ...
        if ( $Self->{Config}->{State} && $GetParam{NewStateID} ) {
            ...
            if ( $StateData{TypeName} =~ /^close/i ) {
        				$Self->{TicketObject}->AuditTicketAction (
										Action		=>	'UPDATE',
										Message		=>	"Added note for TicketID $Self->{TicketID} by UserID $Self->{UserID}.",
										UserID		=>	$Self->{UserID},
										TicketID	=>	$Self->{TicketID},
										Body			=>	'No',
										Result		=>	'OK',
        ...
        #Invio la nota solo se si tratta di una "Nota per Wind"
        if($GetParam{ArticleTypeID} eq $MS_PM_Wind_settings_ptr->{article_typeID_ToWind})
        {
...
sub _Mask {
    ...
    if ( $Self->{Config}->{Note} ) {
		...
		$Self->{LogObject}->Log( Priority => 'debug', Message => "Called AgentTicketNote -> get ArticleTypeList" );
        # get possible notes
        my %DefaultNoteTypes = %{ $Self->{Config}->{ArticleTypes} };
        my %NoteTypes = $Self->{TicketObject}->ArticleTypeList( Result => 'HASH' );
		my %CornerNoteTypes = %{ $Self->{Config}->{CornerArticleTypes} };
		my $currentTheme = $Self->_getTheme();
		if( $currentTheme eq 'Corner'){
        for ( keys %NoteTypes ) {
				if ( !$CornerNoteTypes{ $NoteTypes{$_} } ) {
					delete $NoteTypes{$_};
				}
			}
		}else{
