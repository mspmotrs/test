...
sub Run {
    ...
    if ( !$Access ) {
				$Self->{TicketObject}->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Impossible to close the TicketID $Self->{TicketID}: no $Self->{Config}->{Permission} permissions grant to UserID $Self->{UserID}.",
						UserID		=>	$Self->{UserID},
						TicketID	=>	$Self->{TicketID},
						Body			=>	'No',
						Result		=>	'KO',
        ...
        if ( !$Self->{TicketObject}->LockIsTicketLocked( TicketID => $Self->{TicketID} ) ) {
            ...
            if ( !$AccessOk ) {
                my $Output = $Self->{LayoutObject}->Header( Value => $Ticket{Number} );
            		...
            		$Self->{TicketObject}->AuditTicketAction (
										Action		=>	'UPDATE',
										Message		=>	"Closing ticket action failed because userID $Self->{UserID} is not the owner of the ticket $Self->{TicketID}.",
										UserID		=>	$Self->{UserID},
										TicketID	=>	$Self->{TicketID},
										Body			=>	'No',
										Result		=>	'KO',
    ...
    if ( $Self->{Subaction} eq 'Store' ) {
        ...
        if ( $Self->{Config}->{State} && $GetParam{NewStateID} ) {
            ...
            if ( $StateData{TypeName} =~ /^close/i or ($StateData{TypeName} =~ /^wind/i and ($GetParam{NewStateID} == 15 or $GetParam{NewStateID} == 16)) ) {
                $Self->{TicketObject}->AuditTicketAction (
										Action		=>	'UPDATE',
										Message		=>	"TicketID $Self->{TicketID} has been closed by UserID $Self->{UserID}.",
										UserID		=>	$Self->{UserID},
										TicketID	=>	$Self->{TicketID},
										Body			=>	'No',
										Result		=>	'OK',
