...
sub Run {
    ...
    if ( !$Access ) {
				$Self->{TicketObject}->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Impossible to set TicketID $Self->{TicketID} to a pending status: no $Self->{Config}->{Permission} permissions is granted for the UserID $Self->{UserID}.",
						UserID		=>	$Self->{UserID},
						TicketID	=>	$Self->{TicketID},
						Body		=>	'No',
						Result		=>	'KO',
        ...
        if ( !$Self->{TicketObject}->LockIsTicketLocked( TicketID => $Self->{TicketID} ) ) {
            ...
            if ( !$AccessOk ) {
                my $Output = $Self->{LayoutObject}->Header( Value => $Ticket{Number} );
               	$Self->{TicketObject}->AuditTicketAction (
										Action		=>	'UPDATE',
										Message		=>	"Set in pending action failed because userID $Self->{UserID} is not the owner of the ticket $Self->{TicketID}.",
										UserID		=>	$Self->{UserID},
										TicketID	=>	$Self->{TicketID},
										Body			=>	'No',
										Result		=>	'KO',
    ...
    if ( $Self->{Subaction} eq 'Store' ) {
        ...
        if ( $Self->{Config}->{State} && $GetParam{NewStateID} ) {
            ...
            # set unlock on close ----- MS_Full: lieve modifica per consentire lo stesso trattamento ai nuovi stati
            if ( $StateData{TypeName} =~ /^close/i or ($StateData{TypeName} =~ /^wind/i and ($GetParam{NewStateID} == 15 or $GetParam{NewStateID} == 16)) ) {
                $Self->{TicketObject}->LockSet(
                    TicketID => $Self->{TicketID},
                    Lock     => 'unlock',
                    UserID   => $Self->{UserID},
            ...
            # set pending time ----- MS_Full: lieve modifica per consentire lo stesso trattamento ai nuovi stati
            elsif ( $StateData{TypeName} =~ /^pending/i or ($StateData{TypeName} =~ /^wind/i and ($GetParam{NewStateID} == 13 or $GetParam{NewStateID} == 14))  ) {
            ...
            # redirect to last screen overview on closed tickets ----- MS_Full: lieve modifica per consentire lo stesso trattamento ai nuovi stati
            if ( $StateData{TypeName} =~ /^close/i or ($StateData{TypeName} =~ /^wind/i and ($GetParam{NewStateID} == 15 or $GetParam{NewStateID} == 16)) ) {
     		        $Self->{TicketObject}->AuditTicketAction (
										Action		=>	'UPDATE',
										Message		=>	"TicketID $Self->{TicketID} set in pending status by UserID $Self->{UserID}.",
										UserID		=>	$Self->{UserID},
										TicketID	=>	$Self->{TicketID},
										Body			=>	'No',
										Result		=>	'OK',
        ...
        $Self->{TicketObject}->AuditTicketAction (
				...
				##PM-X22011 Unlock the ticket when the state becomes inAttesaInformazioni, Risolto, nondiCompetenza
				##in modo tale da permettere la presa in gestione e ritornare nello stato inLavorazione
				$Self->{TicketObject}->LockSet(
                    TicketID => $Self->{TicketID},
                    Lock     => 'unlock',
                    UserID   => $Self->{UserID},
    	...
    	# ****** MS changes for CR3264 - file: AgentTicketPending.pm (inizio) *******************************
		use Kernel::Modules::AgentTicketPendingMSAutoMove;
		Kernel::Modules::AgentTicketPendingMSAutoMove::MS_TicketAutoMove(-1, $Self->{TicketID}, -1, -1, -1); #ritorna 1 se ha fatto modifiche al ticket
            ...
            )
        {
            ...
            {   
                ...
                my $MS_RequestType = 'NOTIFY_NO_NEW_ARTICLE';
