...
sub Run {
    ...
    # create new ticket and article
        ...
        if (%Error) {
						...
						$MessageError=substr($MessageError,0,(length $MessageError)-2);
            ...
            my $profile = '';
	          ...
	          if (length $profile > 1) {
		 		    ...
		 		    $Self->{LogObject}->Log (
    		        Priority	=> 'audit',
        		 		Message		=> "Ticket creation for user $Self->{UserID} fails: " . $MessageError,
            		SessionID => '',
		         		User			=> $Self->{UserID},
    		     		Profile		=> $profile,
        		 		Action		=> 'CREATE',
            		TicketId	=> '',
            		Parameter	=> $GetParam{Subject}||'' . '>>' . $GetParam{Body}||'',
		            Result		=> 'KO',
        		);        
        ...
        my $MS_full_TypeID = $GetParam{TypeID};
        my $MS_full_NewQueueID = $NewQueueID;
        if( ($MS_full_NewQueueID eq $MS_PM_Wind_settings_ptr->{queue_id_ericsson})
            and
            ($MS_full_TypeID eq $MS_PM_Wind_settings_ptr->{ticketTypeID_IncidentToWind} or
             $MS_full_TypeID eq $MS_PM_Wind_settings_ptr->{ticketTypeID_IncidentToPM} or
             $MS_full_TypeID eq $MS_PM_Wind_settings_ptr->{ticketTypeID_AlarmToWind} or
             $MS_full_TypeID eq $MS_PM_Wind_settings_ptr->{ticketTypeID_AlarmToPM} 
             )
           ) #se sto CREANDO sulla coda per Ericsson imposto il CHANNEL = Ericcsson
        {
            MS_SetChannel($Self->{DBObject}, $TicketID, 'ERICSSON');
        }
        elsif( $MS_full_TypeID eq $MS_PM_Wind_settings_ptr->{ticketTypeID_IncidentToWind} or
                $MS_full_TypeID eq $MS_PM_Wind_settings_ptr->{ticketTypeID_IncidentToEricsson} or
                $MS_full_TypeID eq $MS_PM_Wind_settings_ptr->{ticketTypeID_AlarmToWind} or
                $MS_full_TypeID eq $MS_PM_Wind_settings_ptr->{ticketTypeID_AlarmToEricsson} 
           ) #se sto CREANDO su qualche coda di PM un tipo di ticket che rientra in ambito FULL: imposto il CHANNEL = PM
        {
            MS_SetChannel($Self->{DBObject}, $TicketID, 'PM');
        }
        # ******** MS full (fine) ********************************************** 
    	...
    	# ****** MS changes for CR3264 - file: AgentTicketPhone.pm (inizio) *******************************
		my $MS_TipoTicket = $GetParam{"TicketFreeText4"};
		my $MS_ArticleType = $Self->{Config}->{ArticleType};
		if($MS_TipoTicket =~ m/^\s*Manutenzione\s+Straordinaria\s*$/i  or 
			$MS_TipoTicket =~ m/^\s*Comunicazioni\s+da\s+Poste\s+Mobile\s*$/i )
		{
			$MS_ArticleType = "note-external-ToCorner";
		
			# TN  
			#[MS 20131010] Attenzione: modifica richiesta in fase di UAT per armonizzare visualizzazione lato DP e OTRS
			$Self->{DBObject}->Do(
						  SQL => "UPDATE ticket SET tn = ? WHERE id = ?",
						  Bind => [ \$TicketID, \$TicketID ],
			);
		}
		# ****** MS changes for CR3264 (fine) ***************************************************************        
        
        my $ArticleID = $Self->{TicketObject}->ArticleCreate(
            NoAgentNotify    => $NoAgentNotify,
            TicketID         => $TicketID,
            ArticleType      => $MS_ArticleType, #MS changes for CR3264 - $Self->{Config}->{ArticleType}
            ...
            Queue => $Self->{QueueObject}->QueueLookup( QueueID => $NewQueueID ),
				...
				if ($TicketID) {
				    $Self->{TicketObject}->AuditTicketAction (
						    Action		=>	'CREATE',
						    Message		=>	"New Ticket (ID=$TicketID) created by user $Self->{UserID}",
						    UserID		=>	$Self->{UserID},
						    TicketID	=>	$TicketID,
						    Body			=>	$GetParam{Body},
						);
...
sub _MaskPhoneNew {
	...
	my $currentTheme = $Self->_getTheme();
    # get list type
    my $TreeView = 0;
    if ( $Self->{ConfigObject}->Get('Ticket::Frontend::ListType') eq 'tree' ) {
        $TreeView = 1;
   ...
   # build type string
    if ( $Self->{ConfigObject}->Get('Ticket::Type') ) {
		...
		if ($Self->{ConfigObject}->Get('Ticket::Type::TicketTypeGroup') && $currentTheme ne 'Corner' ) {

                my %GroupHash;
                my %TicketTypeGroup = %{$Self->{ConfigObject}->Get('Ticket::Type::TicketTypeGroup')};

                foreach my $GroupName (keys %TicketTypeGroup){
				if( $GroupName ne 'Service Request' && $GroupName ne 'Clienti' ) { 
