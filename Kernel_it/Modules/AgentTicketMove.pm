...
sub Run {
    ...
    if ( !$Access ) {
				$Self->{TicketObject}->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Impossible to move TicketID $Self->{TicketID}: no $Self->{Config}->{Permission} permissions is granted for the UserID $Self->{UserID}.",
						UserID		=>	$Self->{UserID},
						TicketID	=>	$Self->{TicketID},
						Body			=>	'No',
						Result		=>	'KO',
    ...
    if ( $Self->{TicketObject}->LockIsTicketLocked( TicketID => $Self->{TicketID} ) ) {
        ...
        if ( !$AccessOk ) {
            my $Output = $Self->{LayoutObject}->Header();
						$Self->{TicketObject}->AuditTicketAction (
								Action		=>	'UPDATE',
								Message		=>	"Moving ticket action failed because userID $Self->{UserID} is not the owner of the ticket $Self->{TicketID}.",
								UserID		=>	$Self->{UserID},
								TicketID	=>	$Self->{TicketID},
								Body			=>	'No',
								Result		=>	'KO',
    ...
    my %MS_PM_Wind_settings = %{$MS_PM_Wind_settings_ptr};
    ...
    #$Self->{LogObject}->Log( Priority => 'notice', Message => "******prova di MOVE ***** QueueID=$Ticket{QueueID} e DestQueueID=$GetParam{DestQueueID})");
    ...
    if (exists($GetParam{DestQueueID}) and
            ...
            and
            exists($Ticket{QueueID})
            and
            $Ticket{QueueID} != $MS_PM_Wind_settings{queue_id_incident_for_wind}
            and
            $Ticket{QueueID} != $MS_PM_Wind_settings{queue_id_ERICSSON_PM_WIND_OUT}
        )
    {
        eval 
        {
            my $MS_current_ticket_TTL_Expired = MSTicketUtil::MS_Is_TTL_Expired_readFromDB($Self->{DBObject}, $Self->{TicketID});
            my $MS_sonoGiaStatoQui = MS_CheckInternalUseField($Self->{DBObject}, $Self->{TicketID});
            $MS_check_OK_for_CREATE = MS_CheckIfIncidentOrSrIsOkForCreate($Self->{TicketID}, 0, $Self->{DBObject}); #MSTicketUtil.pm
            
            $MS_send_result = 0; #setto come KO
            
            #if (exists($Ticket{TicketFreeTime3}) and $Ticket{TicketFreeTime3} ne '')
            if ($MS_sonoGiaStatoQui and !$MS_current_ticket_TTL_Expired and $MS_check_OK_for_CREATE == 1) #si tratta di una REOPEN apparentemente leggittima
            {
                $MS_RequestType = 'REOPEN';
                ...
                #$Self->{LogObject}->Log( Priority => 'notice', Message => "******prova di REOPEN ***** MS_send_result=$MS_send_result");
            }
            elsif($MS_check_OK_for_CREATE == 1) #si tratta di una CREATE apparentemente leggittima
            {
            ...
            if ($MS_send_result == 1) #ok dell'invio della Request verso Wind
            {
                #segno il passaggio sulle code per Wind
                MS_UpdateInternalUseField($Self->{DBObject}, $Self->{TicketID}) if(!$MS_sonoGiaStatoQui);
                
                $Move = $Self->{TicketObject}->MoveTicket(
                QueueID            => $GetParam{DestQueueID},
                UserID             => $Self->{UserID},
                TicketID           => $Self->{TicketID},
                SendNoNotification => $GetParam{NewUserID}, #cosi' era nella move originale
                Comment            => $BodyAsText,
                );
                
                #Cambio il proprietario per impedire l'editabilita' (SOLO per SR/Incident)
                my $MS_changeOwner_result = $Self->{TicketObject}->OwnerSet(
                    TicketID  => $Self->{TicketID},
                    NewUserID => $MS_PM_Wind_settings{system_user_id},
                    UserID    => $MS_PM_Wind_settings{system_user_id}, # -> changed_by
                    SendNoNotification => 1,
                );
            }
            else
            {
                $Self->{LogObject}->Log( Priority => 'error', Message => "$MS_messages->{message} $MS_messages->{message_more}") if(defined($MS_messages) and ref($MS_messages) eq 'HASH' and exists($MS_messages->{message}) and exists($MS_messages->{message_more}) ) ;
            }
        };
        if($@)
        {
           #gestione errore
            $Self->{LogObject}->Log( Priority => 'error', Message => "Errore di CREATE o REOPEN Incident verso Wind");
        }
            ...
            (   $GetParam{DestQueueID} eq $MS_PM_Wind_settings{queue_id_alarm_to_wind} or 
                $GetParam{DestQueueID} eq $MS_PM_Wind_settings{'queue_id_ERICSSON-WIND-OUT-Alarm'}  )
            
            and
            exists($Ticket{QueueID})
            and
            $Ticket{QueueID} != $MS_PM_Wind_settings{queue_id_alarm_to_wind}
            and
            $Ticket{QueueID} != $MS_PM_Wind_settings{'queue_id_ERICSSON-WIND-OUT-Alarm'}
        ) # eliminato -> or $GetParam{DestQueueID} eq $MS_PM_Wind_settings{'queue_id_WIND-ERICSSON-OUT-Alarm'}
    {
        eval 
        {  
            ...
            if ($MS_check_OK_for_CREATE == 1) 
            {
                ...
                {
                    #segno il passaggio sulle code per Wind
                    MS_UpdateInternalUseField($Self->{DBObject}, $Self->{TicketID});
    ...
    #$Self->{LogObject}->Log( Priority => 'notice', Message => "_MS_Full_ @@@@@@@@@@@@@@@ -> Valore di ritorno della move: $Move");
    ...
    if ( !$Move ) {
				$Self->{TicketObject}->AuditTicketAction (
						Action		=>	'UPDATE',
						Message		=>	"Ticket $Self->{TicketID} moving in queue $GetParam{DestQueueID} failed for user $Self->{UserID}.",
						UserID		=>	$Self->{UserID},
						TicketID	=>	$Self->{TicketID},
						Body			=>	'No',
						Result		=>	'KO',
    ...
    $Self->{TicketObject}->AuditTicketAction (
				Action		=>	'UPDATE',
				Message		=>	"TicketID $Self->{TicketID} moved in queue $GetParam{DestQueueID} by UserID $Self->{UserID}.",
				UserID		=>	$Self->{UserID},
				TicketID	=>	$Self->{TicketID},
				Body		=>	'No',
				Result		=>	'OK',
